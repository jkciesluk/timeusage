...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val toKey: (String => String) = (name: String) => {
      if(primary.exists(name.startsWith(_))) "primary"
      else if(work.exists(name.startsWith(_))) "work"
      else if(other.exists(name.startsWith(_))) "other"
      else "unknown"
    }
    val grouped = columnNames.groupBy(toKey).map{case (key, vs) => (key, vs.map(column(_)))}
